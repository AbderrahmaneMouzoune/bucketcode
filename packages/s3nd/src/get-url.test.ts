...
describe('Bucket.getUrl', () => {
  ...
  it('applies a per-call prefix instead of the bucket-level one', async () => {
    ...
    const store = createBucket({ bucket: 'b', prefix: 'transfers', publicUrl: 'https://cdn.example.com', client })

    expect(await store.getUrl('K7QP2M4X')).toBe('https://cdn.example.com/transfers/K7QP2M4X')
