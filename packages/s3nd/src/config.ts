...
function normalizePublicUrl(publicUrl: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new S3ndError('INVALID_CONFIG', `\`publicUrl\` must be an absolute URL (received "${publicUrl}").`)
  }
...
export function resolveConfig(config: BucketConfig = {}): ResolvedConfig {
  const bucket = config.bucket ?? env('S3ND_BUCKET', 'S3_BUCKET')

  if (typeof bucket !== 'string' || bucket.trim().length === 0) {
    throw new S3ndError(
      'INVALID_CONFIG',
      'A bucket name is required. Pass `{ bucket }` to createBucket(), or set S3ND_BUCKET / S3_BUCKET.',
    )
  }

  if (config.maxSize != null && (!Number.isFinite(config.maxSize) || config.maxSize <= 0)) {
    throw new S3ndError('INVALID_CONFIG', '`maxSize` must be a positive number of bytes.')
  }

  const endpoint = config.endpoint ?? env('S3ND_ENDPOINT', 'S3_ENDPOINT')

  return {
    bucket: bucket.trim(),
    // R2 and most S3-compatible endpoints ignore the region but the SDK still
    // requires one; "auto" is the convention.
    region: config.region ?? env('S3ND_REGION', 'AWS_REGION', 'AWS_DEFAULT_REGION') ?? (endpoint ? 'auto' : undefined),
    ...
    publicUrl: normalizePublicUrl(config.publicUrl ?? env('S3ND_PUBLIC_URL', 'S3_PUBLIC_URL')),
