...
- `getUrl(key, options)` — public URL when `publicUrl` is configured, presigned `GET` otherwise,
  with optional `download` disposition.
...
- Uploads transit through your server, so they are bound by your runtime's request size limit
  (6 MB on synchronous Lambda, 4.5 MB on Vercel). See the documentation.
