...
| ---------------------- | ----------------------------------------------------------------- |
| `bucketcode`           | Your server talks to the bucket. Brings the AWS SDK.              |
| `@bucketcode/react`    | Your React app sends or receives transfers. Never pulls the SDK.  |
| `@bucketcode/cli`      | You want the `bucketcode` command. Install it globally.           |
| `@bucketcode/protocol` | You are writing a client or server for a runtime the others miss. |

`@bucketcode/react` depends on `@bucketcode/protocol`, never on `bucketcode`, so a front-end
