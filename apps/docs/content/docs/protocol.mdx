...
`@bucketcode/protocol` is `fetch` and nothing else. Its whole dependency tree is nanoid, and no
...
of `bucketcode`: a subpath would have put the SDK in every front-end's `node_modules`.
