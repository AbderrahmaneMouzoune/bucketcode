...
description: 'Four HTTP routes between a bucketcode server and anything that talks to it — written down, so it is not just whatever the handler happens to do.'
---
...
Rails works with every bucketcode client. And the CLI pointed at `--remote` cannot tell which of
those it is talking to — which is exactly why `bucketcode put` works against your own deployment.
...
`bucketcode/protocol` is `fetch` and nothing else — no path from it reaches the AWS SDK, so it
bundles for a browser, a worker or React Native without dragging a storage client along.
...
filename travels percent-encoded in `X-Bucketcode-Filename`.
...
in your bucket: removing it is an S3 lifecycle rule's job, and bucketcode does not create one for
you. [`bucketcode doctor`](/docs/cli) checks whether you have one, because a bucket quietly filling
