...
const current = await bucket.getSnapshot<AppState>(`user-${userId}`)

try {
  await bucket.putSnapshot(`user-${userId}`, merge(current?.data, incoming), {
...
async function saveWithRetry(userId: string, incoming: AppState, attempts = 3): Promise<void> {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (isBucketCodeError(error) && error.code === 'PRECONDITION_FAILED') continue
      throw error
    }
  }
...
so merges are defined by the data structure, and keep bucketcode as the place its state lives:
