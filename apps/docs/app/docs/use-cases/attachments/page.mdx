...
export async function POST(request: Request) {
  ...
  const result = await bucket.upload(file, { prefix: `attachments/${normalizeSyncCode(code)}` })
...
export async function GET(_request: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  return Response.json({ url: await bucket.getUrl(key, { expiresIn: 300 }) })
}
```
...
await bucket.putSnapshot(code, stateReferencing(keys))
```
...
await bucket.put(`user-${userId}/${hash}`, file, { prefix: 'attachments' })
```
